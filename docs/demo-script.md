...
### 2. Disparar Cobro Duplicado en Spotify (txn_0035 - El Flujo del Video)
```bash
...
```
*(Nota: El botón "Simular aumento (+ $5.00)" de la UI simula una anomalía de incremento de precio, por ejemplo sobre Netflix `sub_001`. Para el flujo narrado en el video con `inv_0034` e `inv_0035` de Spotify, utilice el comando `txn_0035`).*
